TO BUILD AND RUN:
- Run "make" from the directory containing the README
- Run "./cavebat" to run the program
- If in the cave, run "/share/cave/bin/run ./cavebat" to run the program

Use the following controls when in the program on desktop:
- Mouse (standard first person camera):
    - Move the mouse horizontally to yaw the camera (rotate in the horizontal plane)
    - Move the mouse vertically to pitch the camera (look up and down)
- Keyboard:
  - Use W,A,S,D to move forwards, strafe left, move backwards, and strafe right, respectively
  - Press R to reset the beam visualization
  - Press SPACE to pause and play the beam visualization
  - Press F to speed up the visualization and E to slow it down
  - Press H to show only high frequencies, L to show only low frequencies, and B to show both

Use the following controls when in the program in the CAVE:
- Wand:
  - Move the wand joystick forwards or backwards to move the camera in the direction of the laser pointer
  - Move the wand joystick left or right to rotate the camera
  - Press the wand joystick in to toggle frequency filtering
  - Press the top, back wand button to pause/play the simulation
  - Press the bottom, back wand button to reset the simulation

Below are the standard instructions that appear in the vdemo project:
